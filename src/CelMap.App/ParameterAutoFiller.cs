...
public static class ParameterAutoFiller
{
    public static void AutoFill(MappingRowViewModel row, MainViewModel vm, AliasRules aliases)
    {
        ...
        if (aliases.AreAliases(label, "GroupID") && !string.IsNullOrWhiteSpace(vm.GroupIdText))
            row.SetConstant(vm.GroupIdText, MainViewModel.SampleRowCount);
        ...
        {
            if (!string.IsNullOrWhiteSpace(vm.InsurerIdText)) row.SetConstant(vm.InsurerIdText, MainViewModel.SampleRowCount);
        }
