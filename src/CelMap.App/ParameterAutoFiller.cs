...
{
    /// <param name="previewRows">How many rows the constant repeats down in the preview — the
    /// actual data-row count of the source sheet (capped at 100) so parameter columns line up
    /// with mapped columns and never render more rows than the data has.</param>
    public static void AutoFill(MappingRowViewModel row, ParametersViewModel vm, AliasRules aliases, int previewRows)
    {
        string label = row.TargetLabel;

        if (aliases.AreAliases(label, "GroupID") && !string.IsNullOrWhiteSpace(vm.GroupIdText))
            row.SetConstant(NormalizeInt(vm.GroupIdText), previewRows);
        ...
        {
            if (!string.IsNullOrWhiteSpace(vm.InsurerIdText)) row.SetConstant(NormalizeInt(vm.InsurerIdText), previewRows);
        }
        ...
        {
            if (vm.ReviewStart.HasValue) row.SetConstant(vm.ReviewStart.Value.ToString("yyyy-MM-dd"), previewRows);
        }
        ...
        {
            if (vm.ReviewEnd.HasValue) row.SetConstant(vm.ReviewEnd.Value.ToString("yyyy-MM-dd"), previewRows);
        }
        else if (label.Equals("GSC", StringComparison.OrdinalIgnoreCase) || label.Equals("GSC Cover", StringComparison.OrdinalIgnoreCase))
            row.SetConstant(vm.CategoryOverrides.Any(c => c.IsEnabled) ? "[Dynamic GSC]" : (vm.DefaultCoverGSC ? "Y" : "N"), previewRows);
        else if (label.Equals("GL", StringComparison.OrdinalIgnoreCase) || label.Equals("GL Cover", StringComparison.OrdinalIgnoreCase))
            row.SetConstant(vm.CategoryOverrides.Any(c => c.IsEnabled) ? "[Dynamic GL]" : (vm.DefaultCoverGL ? "Y" : "N"), previewRows);
        else if (label.Equals("TPD", StringComparison.OrdinalIgnoreCase) || label.Equals("TPD Cover", StringComparison.OrdinalIgnoreCase))
            row.SetConstant(vm.CategoryOverrides.Any(c => c.IsEnabled) ? "[Dynamic TPD]" : (vm.DefaultCoverTPD ? "Y" : "N"), previewRows);
    }
